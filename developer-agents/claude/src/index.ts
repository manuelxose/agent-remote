...
export function createClaudeAdapter(resolve: Resolver = () => resolveDeveloperExecutable("claude")): DeveloperAgentAdapter {
  ...
  return {
    ...
    async execute(request, context) {
      ...
      const argv = ["-p", "--output-format", "json", "--add-dir", context.workingDirectory, ...(request.model ? ["--model", request.model] : []), ...(request.sessionId ? ["--resume", request.sessionId] : []), "--", request.prompt];
      let process;
      try {
