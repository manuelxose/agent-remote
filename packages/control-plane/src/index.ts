...
export class ControlPlane {
  ...
  async handle(message: Message, identity: ControlPlaneIdentity): Promise<CommandResult> {
    ...
    if (await this.repositories.has(message.id)) {
      await this.publish("message.duplicate", correlationId, { messageId: message.id, ownerId: identity.id });
      return { text: "Duplicate message ignored.", status: "warning" };
    }
    await this.repositories.record({ messageId: message.id, recordedAt: new Date().toISOString(), correlationId, ownerId: identity.id });
    const parsed = this.registry.parse(message.text);
    const session = await this.resolveSession(message, identity);
    if (parsed) {
      ...
      if (!definition) return this.rejected(correlationId, `Unknown command: /${parsed.name}\nUse /help to see available commands.`);
      if (definition.requiresInitialization && !session) return this.rejected(correlationId, "This conversation is not initialized. Use /init first.");
      if (session && definition.states && !definition.states.includes(session.status)) return this.rejected(correlationId, `Command /${definition.name} is not available while this chat is ${session.status.toLowerCase()}.`);
    ...
    if (!session) return this.rejected(correlationId, "This conversation is not initialized. Use /init first.");
    if (session.status === "CLOSED") return this.rejected(correlationId, "This chat is closed. Use /init first.");
    if (!session.activeAgent) return { text: "Select an agent first: /claude, /codex, or /copilot.", status: "warning" };
  ...
  private registerCommands(): void {
    ...
    add("init", "Initialize or reactivate the current managed chat.", "/init [name]", "SESSION", context => this.init(context), { requiresInitialization: false, requiredRole: "operator" });
    ...
    add("rename", "Rename the current managed chat.", "/rename <name>", "SESSION", context => this.rename(context));
    add("close", "Close the current managed chat without deleting provider sessions.", "/close", "SESSION", context => this.close(context), { requiredRole: "owner" });
    add("reset", "Start a fresh provider context for the active agent.", "/reset confirm", "SESSION", context => this.reset(context), { requiredRole: "operator" });
    for (const agent of ["claude", "codex", "copilot"]) add(agent, `Select ${agent} as the active developer agent.`, `/${agent}`, "AGENTS", context => this.selectAgent(context, agent), { requiredRole: "operator" });
    add("agent", "Show the currently selected agent.", "/agent", "AGENTS", async context => ({ text: `Agent: ${context.session?.activeAgent ?? "none"}` }));
    add("model", "Show or change the configured provider model.", "/model [allowed-alias]", "AGENTS", context => this.model(context), { requiredRole: "operator" });
    add("workspace", "Show or change the approved workspace.", "/workspace [use <alias-or-approved-path>]", "WORKSPACE", context => this.workspace(context), { requiredRole: "operator" });
    ...
    add("cancel", "Cancel the current execution.", "/cancel", "EXECUTION", context => this.cancel(context), { requiredRole: "operator" });
    add("retry", "Retry the last failed prompt.", "/retry", "EXECUTION", context => this.retry(context), { requiredRole: "operator" });
  ...
  private async init(context: CommandContext): Promise<CommandResult> {
    ...
    const session: ManagedConversation = existing ?? {
      ...
      workspace: this.resolveWorkspace(this.options.defaultWorkspace)!, providerSessionIds: {}, createdAt: now, updatedAt: now,
  ...
  private async chat(context: CommandContext): Promise<CommandResult> {
    ...
    const sessions = await this.repositories.listByOwner(context.identity.id);
    const selected = sessions.find(item => item.logicalSessionId === context.args || item.displayName.toLowerCase() === context.args.toLowerCase());
  ...
  private async reset(context: CommandContext): Promise<CommandResult> {
    ...
    if (context.session.activeAgent) delete context.session.providerSessionIds[`${context.session.activeAgent}:${context.session.workspace}`];
    if (context.session.activeAgent) await this.options.resetProviderSession?.(context.session, context.session.activeAgent);
    await this.touch(context.session);
  ...
  private async retry(context: CommandContext): Promise<CommandResult> {
    if (!context.session?.lastError || !context.session.activeAgent) return { text: "There is no failed execution to retry.", status: "warning" };
  ...
  private async executePrompt(session: ManagedConversation, message: Message, identity: ControlPlaneIdentity): Promise<CommandResult> {
    ...
    const task = async (signal: AbortSignal): Promise<AgentResponse> => {
      ...
      await this.publish("execution.started", message.id, { logicalSessionId: session.logicalSessionId, agent: agentId });
      ...
      const route: Route = { id: `managed-${session.logicalSessionId}`, runtime: "developer-agent", agent: agentId, workspaceRoot: session.workspace };
      const conversation = { id: session.logicalSessionId, channel: session.channel, participantIds: [identity.id], metadata: {} };
      const context: ConversationContext = { message, conversation, route, execution: { correlationId: message.id, conversationId: session.logicalSessionId, workspaceRoot: session.workspace, signal, metadata: model ? { model: model.providerModel, modelAlias: model.alias } : undefined } };
      const response = await runtime.execute(context, agent);
      if (deferred) await this.options.onExecutionResponse?.(session, response);
      return response;
    };
    const result = queue.enqueue(task);
    if (!result.started && result.position === -1) return { text: "Execution queue is full. Try again later.", status: "warning" };
    if (!result.started) {
      deferred = true;
    ...
    try {
      ...
      session.status = response.metadata?.reason && response.metadata.reason !== "cancelled" ? "ERROR" : "IDLE";
      session.lastError = response.metadata?.reason && response.metadata.reason !== "cancelled" ? response.metadata.reason : undefined;
      if (response.metadata?.sessionId) {
        session.providerSessionIds[`${agentId}:${session.workspace}`] = response.metadata.sessionId;
        await this.repositories.setProviderSession(session.logicalSessionId, agentId, session.workspace, response.metadata.sessionId);
      }
      await this.touch(session);
      await this.publish(response.metadata?.reason ? "execution.failed" : "execution.completed", message.id, { logicalSessionId: session.logicalSessionId, agent: agentId });
      return { text: response.text, metadata: response.metadata };
    } catch {
      session.status = "ERROR";
      session.lastError = "execution-failed";
      await this.touch(session);
      await this.publish("execution.failed", message.id, { logicalSessionId: session.logicalSessionId, agent: agentId });
  ...
  private async resolveSession(message: Message, identity: ControlPlaneIdentity): Promise<ManagedConversation | undefined> {
    ...
    if (chosen && chosen.ownerId === identity.id && chosen.status !== "CLOSED") return chosen;
    return external;
  }
