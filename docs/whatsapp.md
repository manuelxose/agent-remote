...
Set `AGENT_REMOTE_HISTORY_PATH` to choose the local JSONL file for imported WhatsApp chat metadata and message text; it defaults to `data/whatsapp-history.jsonl`. The file is created and enforced with mode `0600`. WhatsApp Web history sync is not a complete backup: WhatsApp decides which chats and messages are available to import, so older content can be missing. The import retains attachment descriptors only; attachment binaries are never downloaded.
...
Imported chats are available through `/chat`: use `/chat` to list them, then `/chat <name-or-id> <question>` to provide a bounded transcript excerpt to the active developer agent. Imported transcript text is untrusted reference data and is explicitly marked as such in the prompt.
