...
test("Claude builds fixed argv, parses JSON, and resumes its native session", async () => {
  ...
  assert.deepEqual(calls[0], { executable: "/bin/claude", argv: ["-p", "--output-format", "stream-json", "--verbose", "--add-dir", process.cwd(), "--", request.prompt], workingDirectory: process.cwd(), signal: context.signal, timeoutMs: 1000, maxOutputBytes: 1000 });
  ...
  assert.deepEqual((calls[1] as any).argv, ["-p", "--output-format", "stream-json", "--verbose", "--add-dir", process.cwd(), "--resume", "claude-session", "--", request.prompt]);
});
...
test("Claude and Codex pass explicitly resolved models to their CLIs", async () => {
  ...
  assert.deepEqual(claudeCalls[0].argv, ["-p", "--output-format", "stream-json", "--verbose", "--add-dir", process.cwd(), "--model", "claude-sonnet-configured", "--", request.prompt]);
